...
class ScaleDotProductAttention(torch.nn.Module):
  ...
  def forward(self, Q:torch.Tensor, K:torch.Tensor, V:torch.Tensor, mask=None):
    ...
    d_out = Q.size(-1)
    A1 = A/torch.div(A, torch.sqrt(torch.tensor(d_out, dtype=torch.float32)))  # [batch_size, T, T]
    # 3, 如果有掩码，则将注意力分数矩阵中对应掩码位置的值设为负无穷大
    if mask is not None:
      A1 = A1.masked_fill(mask == 0, -1e9)
    # 4. A2 = softmax(A1)，对A1每一行进行softmax转换，为每个词在上下文中的注意力系数
    A2 = F.softmax(A1, dim=2) # [batch_size, T, T]
...
class MultiHeadAttention(torch.nn.Module):
  ...
  def __init__(self, n_head:int, d_in:int, d_out:int):
    ...
    self.Wq = torch.nn.Linear(d_in, d_out)  # [d_in, d_out]
    self.Wk = torch.nn.Linear(d_in, d_out)  # [d_in, d_out]
    self.Wv = torch.nn.Linear(d_in, d_out)  # [d_in, d_out]
    self.Wo = torch.nn.Linear(d_out, d_out)  # [d_out, d_out]

  def __split_heads(self, x: torch.Tensor):
    """将张量拆分为多头"""
    batch_size, T, _ = x.shape
    device = x.device
    x = x.view(batch_size, T, self.n_head, self.d_out // self.n_head)
    return x.transpose(1, 2).to(device)  # [batch_size, n_head, T, d_out/n_head]

  def __expand_mask(self, mask: torch.Tensor):
    """扩展掩码以适配多头注意力"""
    expanded_mask = mask.unsqueeze(1).repeat(1, self.n_head, 1, 1)  # [batch, n_head, T, T]
  ...
  def forward(self, Q:torch.Tensor, K:torch.Tensor, V:torch.Tensor, mask=None):
    ...
    if mask is not None:
      multi_head_mask = self.__expand_mask(mask).to(Q.device)
